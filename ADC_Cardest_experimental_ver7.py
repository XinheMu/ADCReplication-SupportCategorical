...
class GridMapper:
    ...
    def map_values(self, Targ):
        ...
        for i, a in enumerate(self.a_values):
            ...
            a = a.to(device)
            idx = torch.bucketize(Targ[:, i].contiguous(), a)
            mapped[:, i] = g[idx]
        return mapped
...
class BayesnetEstimator:
    def __init__(self, bayes_called_attribute, bayes_source_attribute, bayes_assist_attribute, 
        ...
        self.valid_bucket_index = None
        self.valid_bucket_lens = None
    ...
    def train(self, data_used):
        ...
        source_indices = torch.bucketize(source_col.contiguous(), self.source_edges, right=True) - 1
        assist_indices = torch.bucketize(assist_col.contiguous(), self.assist_edges, right=True) - 1
    ...
    def load(self):
        ...
        self.valid_bucket_index = (bucket_lens > 0)
        self.valid_bucket_lens = bucket_lens[self.valid_bucket_index]
    ...
    def get_all_slice_cond_probs(self, queried_range):
        ...
        o_starts = torch.clamp(self.dense_hist_edges[:, :, :-1], min=p1)
        o_ends = torch.clamp(self.dense_hist_edges[:, :, 1:], max=q1)
        
        overlap_lens = torch.clamp(o_ends - o_starts, min=0)
        
        fractions = torch.zeros_like(overlap_lens)
        fractions[self.valid_bucket_index] = overlap_lens[self.valid_bucket_index] / self.valid_bucket_lens
...
class GMM_Estimator:
    def __init__(self, dimension_init, kernels_matrix, 
        ...
        self.means_device=self.means.to(device)
        self.stds_device=self.stds.to(device)
        self.stds_perturbed_device=[self.stds_perturbed[i].to(device) for i in range(0,5)]
        self.gaussian_coefficient_device=[self.gaussian_coefficient[i].to(device) for i in range(0,5)]
        self.weights_device=self.weights.to(device)
    ...
    def predict_analytical(self, queried_rectangle,attributes_not_covered):
        ...
        if len(self.pure_gmm_indices) > 0:
            ...
            pure_gmm_prob = torch.nan_to_num(pure_gmm_prob,nan=0)
                
        # 2. Calculate Hybrid Contributions
        hybrid_probs = torch.ones(self.kernels_num)
        P_matrices=[0]*len(self.bayes_source)
        P_matrices_combined=[0]*len(self.bayes_source)
        v1s_combined=[0]*len(self.bayes_source)
        v2s_combined=[0]*len(self.bayes_source)
        for i, bn in enumerate(self.bayes_estimators):
            ...
            if self.is_rightmost[i]:
                # --- Construct v1 (x1) ---
                idx = (torch.bucketize(torch.tensor([a1,b1]), bn.source_edges, right=True) - 1).clamp(0, bn.S1 - 1)
                idx_start_1, idx_end_1=idx[0], idx[1]
                ...
                if idx_start_1 == idx_end_1:
                    v1[:, idx[0]] = self._integrate_gaussian(a1, b1, m1, s1)
                else:
                    head_high_1 = bn.source_edges[idx_start_1 + 1]
                    tail_low_1 = bn.source_edges[idx_end_1]
                    residuals=self._integrate_gaussian(torch.tensor([[a1],[tail_low_1]]), torch.tensor([[head_high_1],[b1]]), m1, s1)
                    v1[:,idx_start_1]=residuals[0,:]
                    v1[:,idx_end_1]=residuals[1,:]

                # --- Construct v2 (x2) ---
                if bn.S2>1:
                    idx = (torch.bucketize(torch.tensor([a2,b2]), bn.assist_edges, right=True) - 1).clamp(0, bn.S2 - 1)
                else:
                    idx = torch.tensor([0,0])
                idx_start_2, idx_end_2=idx[0], idx[1]
                ...
                if idx_start_2 == idx_end_2:
                    ...
                    head_high_2 = bn.assist_edges[idx_start_2 + 1]
                    tail_low_2 = bn.assist_edges[idx_end_2]
                    residuals=self._integrate_gaussian(torch.tensor([[a2],[tail_low_2]]), torch.tensor([[head_high_2],[b2]]), m2, s2)
                    v2[:,idx_start_2]=residuals[0,:]
                    v2[:,idx_end_2]=residuals[1,:]
    ...
    def predict_and_sample(self,
        ...
        modified_target_rectangle[:,is_numerical]=modified_target_rectangle[:,is_numerical]-0.05*torch.sum(modified_target_rectangle[:,is_numerical]<reg_const_two[:,is_numerical].unsqueeze(1),0)+0.1
        ...
        modified_target_rectangle[:,is_cat]=modified_target_rectangle[:,is_cat]/normalization_scale
        ...
        perturbation_level=torch.sum(torch.prod(smallest_three_edges)>(64*Std_min_cube[0:4]))
        if working_mode=='ADC+':
    ...
    def GMM_OnePointEst(self,
                        ...
                        perturbation_level):
        calc_matrix=(position-self.means_device[:dimension,:].unsqueeze(2))/self.stds_perturbed_device[perturbation_level].unsqueeze(2)
        calc_matrix=0.5*(calc_matrix**2)
        calc_matrix=torch.exp(-1*torch.sum(calc_matrix,0))/(self.gaussian_coefficient_device[perturbation_level])
        return torch.sum(calc_matrix*self.weights_device.unsqueeze(1),0)
...
def set_global(Intervals,TimeAllowed_min,batch_size,dim,cutoff,bayes_source_attributes=None,bayes_called_attributes=None,bayes_assist_attributes=None):
    global histogram, gmm_estimator, length, BatchSize, TimeMax, TimeChart, TimeChart_InNetwork, SignalDecay, SignalDecay_abbrev, NoiseVar, NoiseVar_inv, NoiseVarSquare, NoiseVarSquare_inv, Weight, Weight_sqrt, Weight_used, Weight_term_additional, PointArray, dimension, PointArrayScore, PointArrayScore_double, cutoff_length, cutoff_time, Var_min, Std_min_cube, Time_min, likelihood_formula_fixed_term, residual_tail_adjust_term, normal, log_likelihood_adjust, eval_point_backup, normalization_scale
    ...
    likelihood_formula_fixed_term=dimension*(0.5*ma.log(1/(2*3.14159265))-0.5)-ma.log(1-ma.exp(-2*cutoff_time))*(dimension/2)
    ...
    PointArrayScore=PointArrayScore.to(device)
    TimeChart=torch.tensor(TimeChart)
    SignalDecay=torch.tensor(SignalDecay)
    NoiseVar=torch.tensor(NoiseVar).to(device)
    NoiseVarSquare=torch.tensor(NoiseVarSquare).to(device)
    Time_min=[TimeAllowed_min,TimeAllowed_min*1.5,TimeAllowed_min*2,TimeAllowed_min*3,TimeAllowed_min*4]
    NoiseVarSquare_inv=[(1/(1-torch.exp(-2*(TimeChart+Time_min[i])))).to(torch.float32).to(device) for i in range(0,5)]
    ...
    Std_min_cube=torch.sqrt(torch.tensor(Var_min))**3
    TimeChart_InNetwork=[(TimeChart+Time_min[i]).to(torch.float32).to(device) for i in range(0,5)]
    residual_tail_adjust_term=[(1/NoiseVarSquare[:,BatchSize*cutoff_length:]-(NoiseVarSquare_inv[i])[:,BatchSize*cutoff_length:]).unsqueeze(2).to(torch.float32).to(device) for i in range(0,5)]
    ...
    TimeChart=TimeChart.to(torch.float32).to(device)
    SignalDecay=SignalDecay.to(torch.float32).to(device)
    SignalDecay_abbrev=SignalDecay_abbrev.to(torch.float32).to(device)
    NoiseVar=NoiseVar.to(torch.float32).to(device)
    NoiseVarSquare=NoiseVarSquare.to(torch.float32).to(device)
    Weight=Weight.to(torch.float32).to(device)
    Weight_sqrt=Weight_sqrt.to(torch.float32).to(device)
    Weight_used=Weight_used.to(torch.float32).to(device)
    PointArray=PointArray.to(torch.float32).to(device)
    PointArrayScore=PointArrayScore.to(torch.float32).to(device)
    PointArrayScore_double=PointArrayScore_double.to(torch.float32).to(device)
    ...
    normalization_scale=1*(1+2*(reg_const_one[0,is_cat]>2)+6*(reg_const_one[0,is_cat]>8)+18*(reg_const_one[0,is_cat]>32)+54*(reg_const_one[0,is_cat]>128))
    eval_point_backup=[torch.cat((PointArray,TimeChart_InNetwork[perturbation_level],NoiseVar_inv[perturbation_level],NoiseVarSquare_inv[perturbation_level]),0).unsqueeze(2).repeat(1,1,draws).to(device) for perturbation_level in range(0,5)]
...
def CardEst_Implement(target_rectangle,bayes_source_attributes=None,bayes_called_attributes=None,bayes_assist_attributes=None,attributes_not_covered=None,working_mode='ADC'):
    global variance, counter
    result,position,predictor_is_more_accurate,perturbation_level=gmm_estimator.predict_and_sample(target_rectangle,attributes_not_covered,bayes_source_attributes,bayes_called_attributes,bayes_assist_attributes,working_mode)
    if predictor_is_more_accurate or working_mode=='ADC-':
        return result
    position=position.to(device).unsqueeze(1)
    ...
    position_used=(position*(SignalDecay_abbrev.unsqueeze(2))).unsqueeze(2).expand(-1,-1,BatchSize,-1).reshape(dimension,length*BatchSize,draws)
    eval_point=eval_point_backup[perturbation_level].clone().detach()
    ...
    score_head=torch.cat((evalnet_head(eval_point_head[:,:15*BatchSize,:]),evalnet_head(eval_point_head[:,15*BatchSize:30*BatchSize,:]),evalnet_head(eval_point_head[:,30*BatchSize:,:])),1)
    residual_tail=(evalnet_tail(eval_point_tail)+eval_point_tail[:dimension,:]*(residual_tail_adjust_term[perturbation_level]))
    ...
    log_density=likelihood_formula_fixed_term-eval_value-(torch.sum(position*position,0)).unsqueeze(0)*(1/(2*(1-ma.exp(-2*cutoff_time)))-1/2)
    log_likelihood_adjust_offset=log_likelihood_adjust[1]-perturbation_level*0.5
    log_likelihood_adjust_weight=log_likelihood_adjust[0]/(1.75**perturbation_level)
    log_density[log_density<log_likelihood_adjust_offset]=(log_density[log_density<log_likelihood_adjust_offset]-log_likelihood_adjust_offset)*(1-log_likelihood_adjust_weight)+log_likelihood_adjust_offset
    log_density=log_density-dimension*Time_min[perturbation_level]
    density_diffusion=torch.nan_to_num((torch.exp(log_density)))
    adjust_term=(density_diffusion+1e-7)/(density_KDE+1e-7)
    z=min(torch.mean(adjust_term),40).to("cpu")
    return min(torch.tensor(1),result*z)
...
def CardEst_Implement_Selective(target_rectangle,bayes_source_attributes=None,bayes_called_attributes=None,bayes_assist_attributes=None,nan_to=-100000000,working_mode='ADC'):
    ...
    np.random.seed(123)
    attributes_not_covered=(maxvals<=target_rectangle[1:2,:])*(minvals>=target_rectangle[0:1,:])
    target_rectangle[0:1,:]=(torch.ceil(target_rectangle[0:1,:]/unit_of_variables-1e-6)-0.5)*unit_of_variables
    target_rectangle[1:2,:]=(torch.floor(target_rectangle[1:2,:]/unit_of_variables+1e-6)+0.5)*unit_of_variables
    if not (date_like==[]):
        target_rectangle[:,date_like]=date_to_days(torch.floor(target_rectangle[:,date_like]).to(int)).to(target_rectangle.dtype)+0.5
    estimate_zero=torch.sum(minvals>target_rectangle[1:2,:])+torch.sum(maxvals<target_rectangle[0:1,:])
    if estimate_zero:
        return torch.tensor([nan_queried*nan_nums])
    num_attributes_not_covered=torch.sum(attributes_not_covered)
    if num_attributes_not_covered==dimension_init:
        return torch.tensor([size+nan_queried*nan_nums])
    if num_attributes_not_covered==dimension_init-1:
        queriedatt=torch.argmin(attributes_not_covered*1)
        ...
        if is_cat[j]:
            ...
            for i, val in enumerate(col_str):
                if val.upper() == 'ALLATTRS':
                    numeric_col[i] = 88888888.0*((-1)**(i-1))
                else:
...
def run(dname,uvar,dim_init,kernum,tm,workload_size,bayes_source_attributes=None,bayes_called_attributes=None,bayes_assist_attributes=None,nan_to=-1000000000,working_mode='ADC',output_mode='qerror',threshold=100):
    global timer, current_query, dataset_name, unit_of_variables, dimension, dimension_init, indexes, indexestwo, gridmapper, index_usegridmap, index_usemcvrad, kernels_num, tmin, evalnet_head, evalnet_tail, kernels_matrix, classifier, timeused, j, actualcard, minimum_rad, bayesnet, is_cat, is_numerical, is_binary
    if output_mode=='qerror':
        actualcard=np.load(dataset_name+'/'+dname+'_real_test.npy',allow_pickle=True)
        high_error_list=[]
        dataset_name=dname
    ...
    if ablation_key<0:
        ...
        evalnet_head=torch.load(dataset_name+'/'+dataset_name+str(ablation_key)+'_head.pkl',weights_only=False).to(device)
    evalnet_tail=torch.load(dataset_name+'/'+dataset_name+'_tail.pkl',weights_only=False).to(device)
    if working_mode=='ADC+':
        with open(dataset_name+'/'+'classifier_'+dataset_name+'.pkl', 'rb') as file:
            classifier = pickle.load(file)
    j=0
    min_sel=1/orisize
    gridmapper=GridMapper.load(dataset_name+'/categorical_grids.pt')
    if not has_categorical:
        workloads=pd.read_csv(dataset_name+'/'+dataset_name+'_testset.csv', delimiter=',', dtype=np.float32, header=None)
    else:
        workloads=pd.read_csv(dataset_name+'/'+dataset_name+'_testset.csv', delimiter=',', dtype=str,  header=None)
    workloads=workloads.to_numpy()
    if has_categorical:
        workloads=convert_dataset(dataset_name,workloads)
    else:
        is_cat=[False for i in range(0,dimension_init)]
    set_global(timestep,tmin,32,dimension,cutoff,bayes_source_attributes,bayes_called_attributes,bayes_assist_attributes)
    ...
    estimate_selectivity=np.zeros((workload_size))
    error=np.zeros((workload_size))  
    timestart=time.time()
    timer=np.zeros((workload_size))
    e=1
    for i in range(0,workload_size):
        current_query=i
        if i%1000==999:
            print("Processed "+str(i+1)+" queries")
        target_rectangle=workloads[2*i:2*i+2,:]
        estimate_selectivity[i]=CardEst_Implement_Selective(target_rectangle,bayes_source_attributes,bayes_called_attributes,bayes_assist_attributes,nan_to,working_mode)/orisize
        if output_mode=='qerror':
            actual_selectivity[i]=actualcard[i]/orisize
            e=actual_selectivity[i]
            error[i]=(max(e,min_sel))/max(estimate_selectivity[i],min_sel)
            if max(error[i],1/error[i])>threshold:
                print(i)
                print(e)
                print(estimate_selectivity[i])
                print(workloads[2*i:2*i+2,:])
                high_error_list.append(i)
    print("Finished estimating "+str(workload_size)+" queries, total time cost (in seconds) is:")
    print(time.time()-timestart)
    if output_mode=='qerror':
        QError=np.maximum(error,1/error)
        SortQ=np.sort(QError)
        data=np.column_stack((np.sort(actual_selectivity),actual_selectivity,estimate_selectivity,QError,SortQ))
        df=pd.DataFrame(data,columns=['relseldis','relsel','estsel','Q','SortQ'])
        with pd.ExcelWriter(dataset_name+'/'+'Statistics_'+dataset_name+'.xlsx', engine='openpyxl', mode='a', if_sheet_exists='new') as writer:
            df.to_excel(writer, sheet_name='Q-Error_'+working_mode, index=False)
        np.save(dataset_name+'/'+dataset_name+'_high_error_list.npy',np.array(high_error_list))
        print("The Q-Error Results for Dataset "+dataset_name+" Are:")
        print("GM")
        print(np.exp(np.mean(np.log(QError))))
        print("median")
        print(SortQ[ma.ceil(workload_size/2)])
        print("95th")
        print(SortQ[ma.ceil(workload_size*95/100)])
        print("99th")
        print(SortQ[ma.ceil(workload_size*99/100)])
        print("MAX")
        print(SortQ[workload_size-1])
    if output_mode=='sel':
        data=np.column_stack((np.sort(estimate_selectivity),estimate_selectivity))
        df=pd.DataFrame(data,columns=['estseldis','estsel'])
        with pd.ExcelWriter(dataset_name+'/'+'Statistics_'+dataset_name+'.xlsx', engine='openpyxl', mode='a', if_sheet_exists='new') as writer:
            df.to_excel(writer, sheet_name='Selectivity_'+working_mode, index=False)

if __name__ == "__main__":
    timestep=(np.load('timestep.npy')).tolist()
    cutoff=43
    GPUspeedup=False
    device = torch.device('cuda' if (torch.cuda.is_available() and GPUspeedup) else 'cpu')
    params=['Estimator','higgs','ADC+','qerror',"[1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3]",'7','1/1280','10000','-10000000000',"[]",'False','100','-1','25']
    for i in range(1,len(sys.argv)):
        params[i]=sys.argv[i]
    dataset_name=params[1]
    wmode=params[2]
    omode=params[3]
    uvar=[ast.literal_eval(params[4])]
    dimension_init=int(params[5])
    Time_min=float(Fraction(params[6]))
    workload_size=int(params[7])
    nanto=int(params[8])
    date_like=ast.literal_eval(params[9])
    has_categorical=(params[10]=='True')
    gmmker=1280+640*has_categorical
    errorthreshold=float(params[11])
    ablation_key=int(params[12])
    draws=int(params[13])
    variance=np.zeros((100000))
    counter=0
    ...
    run(dataset_name,uvar,dimension_init,gmmker,Time_min,workload_size,bayes_source_attributes=bsource_attributes,bayes_called_attributes=bcalled_attributes,bayes_assist_attributes=bassist_attributes,nan_to=nanto,working_mode=wmode,output_mode=omode,threshold=errorthreshold)
